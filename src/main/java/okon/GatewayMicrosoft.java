...
public class GatewayMicrosoft implements Gateway {
    private final Connection connection;

    public GatewayMicrosoft(Job job) {
        try {
    ...
    public List<Message> execute(List<String> queries, List<String> headers) {
        ...
        for (int i = 0; i < queries.size(); i++) {
            try(Statement query = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY); ResultSet rs = query.executeQuery(queries.get(i));) {
